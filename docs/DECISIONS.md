...
**What covers the gap now:** the existing on-screen "One moment" under the orb, which says the same thing without claiming comprehension. A3's *goal* stands; only its mechanism is reversed.
...
**Known limit:** on a slow Sarvam turn the elder now hears nothing at all for a second or two. If real-elder testing shows that reads as "it broke", the answer is a non-verbal cue (a soft tone, or the orb animating while busy) — not words, and not words chosen before we have listened.
