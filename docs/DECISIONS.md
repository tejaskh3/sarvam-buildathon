...
## D20 — Photos surface twice: at the opener and again later in the session (supersedes D4)
**Decision (Tejas, 28 Jul, from testing):** a family photo can now appear on any turn, not only as the session opener. The opener behaviour is unchanged; what is new is a second surfacing later in the conversation.
**Why D4 is reversed:** D4 restricted photos to session start because mid-conversation injection "needs topic-relevance logic and risks derailing her tangent". True of an *arbitrary* moment — so this does not pick one. It waits until the conversation has run (turn 4 at the earliest), never fires during a stalled memory or alongside a family reminder, and asks the model in the prompt to finish her current thought before turning to the picture. The practical failure D4 caused: a family uploads a photo *during* a session and it cannot appear until tomorrow.
**Timing:** eligible from turn 4, 40% per eligible turn so it is not always the same beat, forced by turn 8 so a session with photos waiting never ends without them. Constants at the top of the photo section in `server.js`.
**Two bugs this shipped and then fixed, both found by testing behaviour rather than plumbing:**
1. The instruction was folded into `turnContext`, which is null until she has memories on file — so for a brand-new elder the image appeared on screen with the model never told about it, and the photo was marked SHOWN on the way past. A silent unexplained picture is the "bare image is a quiz" failure the whole photo design exists to prevent.
2. Even once delivered, the model *ignored* it: as a system message it sat behind SYSTEM_PROMPT's twelve rules, and rules 10 (35-word cap) and 12 (never name a place or person she has not mentioned) both forbid describing a photo. Confirmed present in context, on `sarvam-105b`, and still dropped. It now goes in as `trailing` — a message placed after the history — which is the same recency the opener has always had, plus explicit one-turn exemptions from rules 10 and 12.
**Cost:** a photo turn runs on `sarvam-105b` with a 320-token cap, like the stall path. Rare enough to afford.
**Still open — pre-existing, not introduced here:** the deceased gate is a prompt rule, and it leaks. A photo whose subject is flagged `deceased: true` produced the opener *"Aapki maa aaj aapke saath hain, yeh kaisa lag raha hai?"* — present tense, inviting her to feel her dead mother is present. The `SAAVDHAN` wording is byte-identical to what D2 shipped, so this is old, but D2 claimed the failure was made "structurally impossible" and it is not: the upload gate is structural, the *speech* gate is only a prompt. This wants a code-level guard like the recall-test one in `voice.js` — see [[D2]].
